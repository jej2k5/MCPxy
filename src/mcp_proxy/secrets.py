...
The original ``mcp_proxy.secrets`` module shipped a self-contained
file-based store: a Fernet-encrypted JSON blob plus a per-instance key
loader. That code path moved into :class:`mcp_proxy.storage.ConfigStore`
when secrets joined the rest of MCPy's persistent state in a single
SQLAlchemy database.
...
against the database resolved by ``MCPY_DB_URL`` (default:
``sqlite:///<state_dir>/mcpy.db``). The async ``set``/``delete`` shape
...
def load_fernet(
    ...
    *,
    ...
    Order: explicit override → ``MCPY_SECRETS_KEY`` env var → file at
    ...
    def __init__(
        ...
        *,
        ...
        if config_store is not None:
            ...
            url = db_url or os.getenv("MCPY_DB_URL") or _default_db_url_for_state_dir(self.state_dir)
            try:
