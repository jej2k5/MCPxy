...
way to point MCPy at a non-default database was the ``MCPY_DB_URL``
...
2. ``MCPY_DB_URL`` environment variable — stays the operator override
...
def load_bootstrap(state_dir: Path | str) -> BootstrapConfig | None:
    ...
    only uses ``MCPY_DB_URL``) and must not raise. A corrupt file is
