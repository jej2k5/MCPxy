...
This module implements the pieces MCPy needs to act as an OAuth 2.1
...
2. **Dynamic client registration** (RFC 7591, optional). If the config
   ...
   the resulting client_id/secret as a regular MCPy secret so subsequent
...
5. **Persistent storage**. Tokens and dynamic registrations live inside
   the same :class:`mcp_proxy.secrets.SecretsManager` used for user
...
from mcp_proxy.secrets import SecretsManager, SecretNotFoundError
...
class OAuthManager:
    ...
    async def _ensure_client_credentials(
        ...
        body: dict[str, Any] = {
            "client_name": f"MCPy Proxy ({upstream})",
