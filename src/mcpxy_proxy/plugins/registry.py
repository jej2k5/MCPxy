...
from mcpxy_proxy.telemetry.http_sink import HttpTelemetrySink
...
class PluginRegistry:
    ...
    def load_entry_points(self, *, allow_overrides: bool = False) -> None:
        ...
        built-in are silently skipped so MCPxy's own pyproject entry points
        don't collide with the built-ins registered in ``__init__``.
        """
        for ep in entry_points(group="mcpxy_proxy.upstreams"):
            plugin = ep.load()
        ...
        for ep in entry_points(group="mcpxy_proxy.telemetry_sinks"):
            plugin = ep.load()
