...
from mcpxy_proxy.config import AppConfig, load_config, resolve_admin_token
from mcpxy_proxy.discovery.catalog import load_catalog
...
from mcpxy_proxy.stdio_adapter import run_stdio_adapter
from mcpxy_proxy.storage.bootstrap import BootstrapError, load_bootstrap
from mcpxy_proxy.storage.config_store import ConfigStore, open_store
from mcpxy_proxy.storage.db import _default_state_dir, sanitize_url
...
def _bootstrap_config_payload(
    ...
    - ``"default"``            — neither the DB nor a seed file is
                                  ...
                                  API or ``mcpxy-proxy register`` next.
    """
...
def build_state(config_path: str | None) -> AppState:
    ...
    file (or ``MCPXY_SECRETS_KEY``) is unchanged — see ``secrets.py``
    for the rationale.
    """
    state_dir_override = os.getenv("MCPXY_STATE_DIR")
    ...
    #   1. MCPXY_DB_URL env var (operator override in container deploys)
    ...
    # invokes internally.
    db_url_env = os.getenv("MCPXY_DB_URL")
    bootstrap_cfg = None
    if not db_url_env:
        try:
            ...
            raise
    if db_url_env:
        db_url_source = "env:MCPXY_DB_URL"
...
def _expand_for_bootstrap(
    ...
    """
    from mcpxy_proxy.config import _apply_expansions
...
def _resolve_tls_settings(
    ...
    3. Default — MCPxy serves HTTPS out of the box using an
       auto-generated self-signed cert cached under
       ``<state_dir>/tls/``. See :func:`mcpxy_proxy.tls.ensure_dev_cert`.
    ...
    from mcpxy_proxy.storage.db import _default_state_dir
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    uvicorn.run(
        ...
        # traffic recorder. Zero code in MCPxy reads forwarded headers
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="MCPxy multi-upstream MCP proxy")
    ...
    serve.add_argument(
        ...
        action="store_true",
        help=(
            "Serve plain HTTP instead of the default HTTPS. MCPxy serves "
    ...
    init = subparsers.add_parser("init", help="Generate a starter MCPxy config file")
    ...
    install.add_argument(
        "--proxy-command",
        default=None,
        help="Override the `mcpxy-proxy` command path used by stdio adapters",
    )
    ...
    register = subparsers.add_parser(
        "register",
        help="Register a new upstream on a running MCPxy proxy",
    )
    register.add_argument("--url", default="http://127.0.0.1:8000", help="MCPxy proxy base URL")
    ...
    unregister = subparsers.add_parser(
        "unregister",
        help="Remove an upstream from a running MCPxy proxy",
    )
    unregister.add_argument("--url", default="http://127.0.0.1:8000", help="MCPxy proxy base URL")
    ...
    importer = subparsers.add_parser(
        "import",
        help="Import MCP servers from another client into a running MCPxy proxy",
    )
    ...
    importer.add_argument("--url", default="http://127.0.0.1:8000", help="MCPxy proxy base URL")
    ...
    catalog_install = catalog_sub.add_parser(
        "install", help="Install a catalog entry as an upstream on a running MCPxy proxy"
    )
    ...
    catalog_install.add_argument("--url", default="http://127.0.0.1:8000", help="MCPxy proxy base URL")
    ...
    secrets_set.add_argument(
        "--value",
        default=None,
        help="Secret value (default: read from MCPXY_SECRET_VALUE env or stdin)",
    )
...
def _open_local_store() -> ConfigStore:
    state_dir = (
        Path(os.getenv("MCPXY_STATE_DIR")) if os.getenv("MCPXY_STATE_DIR") else _default_state_dir()
    )
    fernet = load_fernet(state_dir)
    return open_store(os.getenv("MCPXY_DB_URL"), fernet=fernet)
...
def cmd_secrets_set(args: argparse.Namespace) -> int:
    ...
    if value is None:
        value = os.getenv("MCPXY_SECRET_VALUE")
    if value is None:
        if sys.stdin.isatty():
            print(
