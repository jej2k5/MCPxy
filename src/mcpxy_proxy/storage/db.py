...
(``pip install mcpxy-proxy[postgres]`` or ``[mysql]``) and point either
``MCPXY_DB_URL`` or the onboarding wizard at it.
"""
...
DEFAULT_SQLITE_FILENAME = "mcpxy.db"
...
# Duplicated from mcpxy_proxy.secrets to avoid an import cycle at module load
...
def _default_state_dir() -> Path:
    ...
    version) so this module has zero imports from other MCPxy modules at
    load time. The two paths are identical in practice.
    """
    override = os.getenv("MCPXY_STATE_DIR")
    if override:
        path = Path(override)
...
def resolve_database_url(
    ...
    *,
    ...
    3. ``<state_dir>/bootstrap.json`` ``db_url`` — written by the
       ...
       via the UI.
    4. ``sqlite:///<state_dir>/mcpxy.db`` default.
    ...
    'sqlite:///foo.db'
    >>> import os; os.environ.pop("MCPXY_DB_URL", None)
    >>> resolve_database_url(None).startswith("sqlite:///")
    True
    """
    if url is not None:
        return url
    env_url = os.getenv("MCPXY_DB_URL")
    if env_url:
        return env_url
    resolved_state_dir = (
...
def available_dialects() -> list[str]:
    ...
    (``pip install mcpxy-proxy[postgres]`` or ``[mysql]``). The wizard
...
def probe_connection(url: str) -> str:
    ...
    if canonical == "sqlite" and (parsed.database in (None, "", ":memory:")):
        raise DatabaseError(
            ...
            "sqlite:///<state_dir>/mcpxy.db instead."
        )
    if canonical in _DIALECT_DRIVER_MODULES and canonical not in available_dialects():
        raise DatabaseError(
            ...
            f"'mcpxy-proxy[{canonical}]' (or the underlying package: "
