...
that can change under refresh. See :mod:`mcpxy_proxy.auth.oauth` for that
...
def build_strategy(cfg: HttpAuthConfig | None) -> AuthStrategy:
    ...
    :class:`mcpxy_proxy.auth.oauth.OAuthAuthStrategy` instead, which
    ...
    if isinstance(cfg, OAuth2AuthConfig):
        raise NotImplementedError(
            ...
            "mcpxy_proxy.auth.oauth.OAuthTokenProvider at transport setup"
        )
