...
# ---------------------------------------------------------------------------
#
# Every test gets its own ``MCPY_STATE_DIR`` pointing at a fresh tempdir,
# plus a fresh randomly generated ``MCPY_SECRETS_KEY``. Without this,
# every test shares the production default ``/var/lib/mcpy`` and stomps
...
# explicitly via ``monkeypatch.setenv("MCPY_STATE_DIR", ...)`` inside
# the test, which takes precedence because it runs after the fixture
# below sets it.


@pytest.fixture(autouse=True)
def _isolated_mcpy_state_dir(monkeypatch):
    from cryptography.fernet import Fernet

    tmp = tempfile.TemporaryDirectory(prefix="mcpy-test-state-")
    monkeypatch.setenv("MCPY_STATE_DIR", tmp.name)
    monkeypatch.setenv("MCPY_SECRETS_KEY", Fernet.generate_key().decode("ascii"))
    # ``MCPY_DB_URL`` defaults to sqlite:///<state_dir>/mcpy.db, so
