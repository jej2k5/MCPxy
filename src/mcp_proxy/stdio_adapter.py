"""Stdio MCP server that forwards JSON-RPC to a running MCPy HTTP proxy.

Used by Claude Desktop and other stdio-only MCP clients to talk to MCPy
without needing a separate process or HTTP transport in the client.

Reads NDJSON from stdin, posts each line to MCPy's /mcp (or /mcp/{upstream})
...
async def _read_lines() -> "asyncio.Queue[str]":
    ...
    threading.Thread(target=_reader, name="mcpy-stdio-reader", daemon=True).start()
    return queue
...
async def _forward(
    ...
    The MCPy proxy answers JSON-RPC requests as NDJSON (one response per
