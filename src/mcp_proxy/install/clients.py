"""Client adapters for installing MCPy into MCP-aware applications.
...
- ClaudeDesktopAdapter — Anthropic's Claude Desktop app, which only
  supports stdio MCP servers. We register `mcp-proxy stdio --connect URL`
...
class InstallOptions:
    ...
    proxy_command: str | None = None  # Override for `mcp-proxy` binary path
...
class ClientAdapter(ABC):
    ...
    def merge(
        ...
        """Return a new config dict with the MCPy entry merged in."""
...
class ClaudeDesktopAdapter(ClientAdapter):
    ...
    def format_entry(self, opts: InstallOptions) -> dict[str, Any]:
        cmd = opts.proxy_command or "mcp-proxy"
